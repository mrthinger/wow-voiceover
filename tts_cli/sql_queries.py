...
def query_dataframe_for_all_quests_and_gossip_localized(language_code : str):
	...
	FROM ALL_DATA
		...
		LEFT JOIN mangos.locales_creature       lc  ON lc .entry = id AND ALL_DATA.type = 'creature'
...
SELECT
	ALL_DATA.*,
	CASE 
		WHEN lq.title_loc3 IS NULL THEN quest_title
		WHEN lq.title_loc3 = '' THEN quest_title
		ELSE lq.title_loc3
	END as "quest_title_de",
	CASE 
		WHEN source = 'accept' THEN IFNULL(lq.Details_loc3, ALL_DATA.text)
		WHEN source = 'progress' THEN IFNULL(lq.RequestItemsText_loc3, ALL_DATA.text)
		ELSE IFNULL(lq.OfferRewardText_loc3, ALL_DATA.text)
	END as "text_de",
	CASE 
		WHEN lc.name_loc3 = '' THEN name
		ELSE IFNULL(lc.name_loc3, name)
	END as "name_de"
FROM
	ALL_DATA
